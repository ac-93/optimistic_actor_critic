...
from optimistic_actor_critic.array_observation.oac_cont.core import *
...
def oac(env_fn, actor_critic=mlp_actor_critic,
    ...
    with tf.variable_scope('main'):
        mu, pi, logp_pi, q1_a, q2_a, pretanh_mu, std  = actor_critic(x_ph, a_ph, **network_params)
    ...
    with tf.variable_scope('main', reuse=True):
        ...
        _, pi_next, logp_pi_next, _, _, _, _ = actor_critic(x2_ph, a_ph, **network_params)

    # Target value network
    with tf.variable_scope('target'):
        _, _, _, q1_pi_targ, q2_pi_targ, _, _ = actor_critic(x2_ph, pi_next, **network_params)
...
if __name__ == '__main__':
    ...
    logger_kwargs = setup_logger_kwargs(exp_name='oac_cont_' + rl_params['env_name'], seed=rl_params['seed'], data_dir=saved_model_dir, datestamp=False)
    env = gym.make(rl_params['env_name'])

    oac(lambda:env, actor_critic=rl_params['actor_critic'],
