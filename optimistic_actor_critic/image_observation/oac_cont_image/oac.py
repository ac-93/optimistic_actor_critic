...
from optimistic_actor_critic.image_observation.oac_cont_image.common_utils import *
from optimistic_actor_critic.image_observation.oac_cont_image.core import *
...
def oac(env_fn, logger_kwargs=dict(), network_params=dict(), rl_params=dict()):
    ...
    with tf.variable_scope('main'):
        mu, pi, logp_pi, q1_a, q2_a, pretanh_mu, std = build_models(x_ph, a_ph, act, act_dim, network_params)

    with tf.variable_scope('main', reuse=True):

        # compose q with mu, for optimistic sampling
        _, _, _, q1_mu, q2_mu, _, _ = build_models(x_ph, mu, act, act_dim, network_params)

        # compose q with pi, for pi-learning
        _, _, _, q1_pi, q2_pi, _, _ = build_models(x_ph, pi, act, act_dim, network_params)

        # get actions and log probs of actions for next states, for Q-learning
        _, pi_next, logp_pi_next, _, _, _, _ = build_models(x2_ph, a_ph, act, act_dim, network_params)

    # Target value network
    with tf.variable_scope('target'):
        _, _, _, q1_pi_targ, q2_pi_targ, _, _ = build_models(x2_ph, pi_next, act, act_dim, network_params)
    ...
    if use_opt:
        ...
        denom = tf.math.sqrt(
            tf.math.reduce_sum(
                tf.math.multiply(tf.math.pow(grad_Q_UB, 2), Sigma)
            )
        ) + 10e-6

        mu_C = np.sqrt(2.0 * delta) * tf.math.multiply(Sigma, grad_Q_UB) / denom
...
if __name__ == '__main__':
    ...
    rl_params = {
        ...
        'use_opt':False,
        'beta_UB': 4.66,
        'beta_LB': -3.65, # -1 to recover standar SAC lower bound
        'delta':23.53
    }

    saved_model_dir = '../../saved_models'
    logger_kwargs = setup_logger_kwargs(exp_name='oac_cont_image_' + rl_params['env_name'], seed=rl_params['seed'], data_dir=saved_model_dir, datestamp=False)
